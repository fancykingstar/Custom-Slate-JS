...
const withPeopleElement = (editor: ReactEditor): ReactEditor => {
  ...
  editor.insertBreak = (): void => {
    ...
    // Do nothing if we're not in the people tool
    if (!inRootElementOfType(editor, PeopleElement.Tool)) {
      insertBreak();
      return;
    }
    ...
    if (node.type === PeopleElement.Team) {
      const nextNode = Editor.next(editor, {
        at: nodePath,
      });
      const nextNodeIsTeam =
        nextNode != null && nextNode[0].type === PeopleElement.Team;
    ...
    if (node.type === PeopleElement.Item) {
  ...
  editor.deleteBackward = (unit) => {
    ...
    if (atRoot) {
      ...
      if (
        ...
        if (prevParentNode.type === PeopleElement.Tool) {
    ...
    if (nodeIsFirstChild && selectionAtBlockStart) {
      if (parentNode.children.length <= 1) {
        Transforms.unwrapNodes(editor, {
          match: (n) => n.type === PeopleElement.Tool,
          split: true,
        });
  ...
  editor.normalizeNode = (entry) => {
    ...
    if (
      ...
      path.length > 0 &&
      (node.type === PeopleElement.Team || node.type === PeopleElement.Item)
    ) {
      const parentPath = Path.parent(path);
      ...
      // Rule: If a parent exists and it's not the people tool, convert to paragraph
      if (
        !Editor.isEditor(parentNode) &&
        parentNode.type !== PeopleElement.Tool
      ) {
        Transforms.setNodes(
          editor,
          {
    ...
    if (
      Editor.isBlock(editor, node) &&
      node.type === PeopleElement.Tool &&
      children != null
    ) {
      // Rule: Tool must always have a first-child choice element
      const firstChild = children[0];
      if (firstChild.type !== PeopleElement.Team) {
...
export default withPeopleElement;
