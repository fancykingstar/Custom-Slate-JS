...
function indentChoice(editor: Editor): void {
  ...
  // We only care about goals elements (e.g., not nested list items)
  if (itemNode.type !== GoalsElement.Item) {
    return;
  }
...
export function unindentGoalSublist(editor: Editor): void {
  const { selection } = editor;
  if (selection == null) {
    return;
  }
  ...
  if (listParentNode.type !== GoalsElement.Item) {
    return;
  }
