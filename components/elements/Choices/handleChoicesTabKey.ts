...
function indentChoice(editor: Editor): void {
  ...
  // We only care about choices elements (e.g., not nested list items)
  if (itemNode.type !== ChoicesElement.Item) {
    return;
  }
...
export function unindentChoiceSublist(editor: Editor): void {
  const { selection } = editor;
  if (selection == null) {
    return;
  }
  ...
  if (listParentNode.type !== ChoicesElement.Item) {
    return;
  }
